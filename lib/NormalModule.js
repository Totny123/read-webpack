...
class NormalModule extends Module {
	...
	doBuild(options, compilation, resolver, fs, callback) {
		...
		const processResult = (err, result) => {
			...
			// extraInfo.webpackAST是result[2].webpackAST，也就是说loader要返回ast？
			// 并不是，假如Loader提供了ast，webpack调用parse()时不使用文件源码，而是使用_ast
			// 换句话说 _ast比文件源码优先级更高
			this._ast =
		...
		// 执行loader。runLoaders内部执行readResource()读取文件内容
		runLoaders(
			{
	...
	build(options, compilation, resolver, fs, callback) {
		...
		return this.doBuild(options, compilation, resolver, fs, err => {
			...
			let result;
			try {
				// this._source.source()拿到文件内容parser成ast
				// 在lib\NormalModuleFactory.js的resolve.tapAsync钩子注入了parser
				result = this.parser.parse(this._ast || this._source.source(), {
					current: this,
					// JavascriptParser.js解析到依赖会给module的dependencies添加
					// 具体代码:parser.state.module.addDependency(sideEffectDep)
					module: this,
